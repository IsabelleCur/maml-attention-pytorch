...
def main():
    ...
    for epoch in range(args.epoch // 4000):
        ...
        for step, (x_spt, y_spt, x_qry, y_qry) in enumerate(db):  # 从get_item出来
            ...
            # print(x_spt)
            # print(x_spt.shape)
            # print(x_spt.is_contiguous())

            # print("===shape====")
            # print("before", x_spt.shape)
            x_spt = torch.tensor(np.array(x_spt).transpose(3, 0, 1, 2, 4))
            x_qry = torch.tensor(np.array(x_qry).transpose(3, 0, 1, 2, 4))
            x_spt = x_spt[0].type(torch.complex64) + 1j * x_spt[1].type(torch.complex64)
            x_qry = x_qry[0].type(torch.complex64) + 1j * x_qry[1].type(torch.complex64)
            # print("medium", x_spt.shape)
            # x_spt.unsqueeze(3)
            # x_qry.unsqueeze(3)
            x_spt = x_spt.view(x_spt.shape[0], x_spt.shape[1], x_spt.shape[2], 1, x_spt.shape[3])
            x_qry = x_qry.view(x_qry.shape[0], x_qry.shape[1], x_qry.shape[2], 1, x_qry.shape[3])
            x_spt, y_spt, x_qry, y_qry = x_spt.to(device), y_spt.to(device), x_qry.to(device), y_qry.to(device)
            # print("after", x_spt.shape)
            # x_spt.transpose(1, 2, 3, 0, 4)
            # x_qry.transpose(1, 2, 3, 0, 4)
            # print(x_spt)
            ...
            if step % 500 == 0:  # evaluation
                ...
                for x_spt, y_spt, x_qry, y_qry in db_test:
                    ...
                    print("===shape====")
                    print("before", x_spt.shape)
                    x_spt = torch.tensor(np.array(x_spt.squeeze(0)).transpose(2, 0, 1, 3))
                    x_qry = torch.tensor(np.array(x_qry.squeeze(0)).transpose(2, 0, 1, 3))
                    x_spt = x_spt[0].type(torch.complex64) + 1j * x_spt[1].type(torch.complex64)
                    x_qry = x_qry[0].type(torch.complex64) + 1j * x_qry[1].type(torch.complex64)
                    print("medium", x_spt.shape)
                    x_spt = x_spt.view(x_spt.shape[0], x_spt.shape[1], 1, x_spt.shape[2])
                    x_qry = x_qry.view(x_qry.shape[0], x_qry.shape[1], 1, x_qry.shape[2])
                    x_spt, y_spt, x_qry, y_qry = x_spt.to(device), y_spt.to(device), x_qry.to(device), y_qry.to(device)
                    print("after", x_spt.shape)
                ...
                if col == 256:
                    index = index + 1;
                    col = 0
                #workbook.save("C1_1shot_comp_atten_maml.xls")  # 保存工作簿
                print('Test acc:', accs)
                #不一定跑完 所以每一次运算后输出
